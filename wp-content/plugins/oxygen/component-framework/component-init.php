...
function ct_init_codemirror_theme() {
	...
	$codemirror_theme = get_option("oxygen_vsb_codemirror_theme", 'dracula');
...
function ct_init_options_white_list() {
	...
	foreach ($options as $option) {
		if (strpos($option, "-unit") === false) {
            $unit = $option."-unit";
			// only add units that exist in defaults to reduce array size and not add things like "color-unit" 
            if (isset($all_defaults[$unit])) {
                $units[] = $unit;
            }
		}
	}

    //$options = array_merge($options, $units);
...
function ct_generate_class_states_css( $class, $state, $options, $is_media = false, $is_selector = false ) {
	...
	if ( isset($options['gap']) ) {
		$pre_styles .= "gap:" . $options['gap'].$options['gap-unit'] . ";\r\n";
	}
